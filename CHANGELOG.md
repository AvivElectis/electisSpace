...
### Fixed
- **Labels page preview images stuck blank** — the lazy-loading effect in `LabelImagePreview` listed `isLoading` and `hasError` in its dependency array. The effect's first run started the AIMS fetch and synchronously flipped `isLoading` to `true`; that re-ran the effect, which fired its cleanup and aborted the very fetch it had just started. With the AbortController tripped before the response landed, `setImageUrl` never ran and `setIsLoading(false)` was skipped (it sits behind `if (!signal?.aborted)`), so every row stayed on the loading skeleton. Removed the self-triggering deps so the controller is only torn down on unmount, label change, or visibility change.
