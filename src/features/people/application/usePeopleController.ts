...
import { parsePeopleCSV, postPersonAssignment, postBulkAssignments } from '../infrastructure/peopleService';
...
export function usePeopleController() {
    ...
    /**
     * Assign space to person
     */
    const assignSpaceToPerson = useCallback(async (
        personId: string,
        spaceId: string,
        postToAims: boolean = false
    ): Promise<void> => {
        try {
    ...
    const bulkAssignSpaces = useCallback(async (
        ...
        try {
            ...
            // Optionally post to AIMS
            if (postToAims && settings.solumConfig && settings.solumConfig.tokens) {
                const assignedPeople = peopleStore.people.filter(p =>
     ...
     */
    const postSelectedToAims = useCallback(async (personIds: string[]): Promise<void> => {
        try {
