/**
 * NativeStoreFeaturesPage — Store-level feature overrides.
 */
...
export function NativeStoreFeaturesPage() {
        ...
        if (sid && companyId) {
            ...
            companyService.getStores(companyId, { limit: 200 })
                .then((resp) => {
                    ...
                    if (found) {
                        if (found.storeFeatures) {
                            setOverrideEnabled(true);
                            setFeatures(found.storeFeatures);
                            setSpaceType(found.storeSpaceType ?? 'office');
                        }
                    }
                })
                .catch(() => {})
    ...
    const handleSave = async () => {
        ...
        setError(null);
        try {
            await companyService.updateStore(sid, {
                storeFeatures: overrideEnabled ? features : null,
                storeSpaceType: overrideEnabled ? spaceType : null,
            });
            navigate(-1);
        } catch (err: any) {
    ...
    return (
        <NativeFormPage
            title={t('settings.stores.overrideCompanyFeatures', 'Override Company Features')}
            onSave={handleSave}
            isSaving={saving}
        >
            {error && (
            ...
            <NativeFormSection title={t('settings.stores.featureOverrides', 'Feature Overrides')}>
                <FormControlLabel
                    control={
                        <Switch
